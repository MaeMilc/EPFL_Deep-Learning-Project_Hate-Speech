...
class CustomClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.dense2 = nn.Linear((2*config.hidden_size), (4*config.hidden_size))
        self.dense3 = nn.Linear(4*(config.hidden_size), config.num_labels)
        classifier_dropout = (
            config.classifier_dropout if config.classifier_dropout is not None else config.hidden_dropout_prob
        )
        self.dropout = nn.Dropout(classifier_dropout)
        self.normalize1 = nn.LayerNorm((2*config.hidden_size), eps=config.layer_norm_eps)
        self.normalize2 = nn.LayerNorm((4*config.hidden_size), eps=config.layer_norm_eps)
